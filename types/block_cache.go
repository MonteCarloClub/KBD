...
func (bc *BlockCache) Clear() {
	bc.blocks = make(map[common.Hash]*Block)
	bc.hashes = nil

}

func (bc *BlockCache) Push(block *Block) {
	bc.mu.Lock()
	defer bc.mu.Unlock()
...
func (bc *BlockCache) Get(hash common.Hash) *Block {
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockCache) Each(cb func(int, *Block)) {
	bc.mu.Lock()
	defer bc.mu.Unlock()
