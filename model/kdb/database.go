...
func (self *LDBDatabase) Close() {
	if err := self.Flush(); err != nil {
		klog.Infof("error: flush '%s': %v\n", self.fn, err)
	}
	self.db.Close()
	klog.Info("flushed and closed db:", self.fn)
}
...
func (self *LDBDatabase) meter(refresh time.Duration) {
	...
	for i := 1; ; i++ {
		...
		if err != nil {
			klog.Infof("failed to read database stats: %v", err)
			return
		}
		...
		if len(lines) <= 3 {
			klog.Infof("compaction table not found")
			return
		}
		lines = lines[3:]
		...
		for _, line := range lines {
			...
			for idx, counter := range parts[3:] {
				if value, err := strconv.ParseFloat(strings.TrimSpace(counter), 64); err != nil {
					klog.Infof("compaction entry parsing failed: %v", err)
					return
				} else {
