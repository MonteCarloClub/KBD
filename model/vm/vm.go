...
func (self *Vm) Run(context *Context, input []byte) (ret []byte, err error) {
	...
	for {
		...
		case LOG0, LOG1, LOG2, LOG3, LOG4:
			...
			log := state2.NewLog(context.Address(), topics, data, self.env.BlockNumber().Uint64())
			self.env.AddLog(log)
...
func (self *Vm) calculateGasAndSize(context *Context, caller ContextRef, op OpCode, statedb *state2.StateDB, mem *Memory, stack *stack) (*big.Int, *big.Int, error) {
	var (
		gas                 = new(big.Int)
...
func (self *Vm) log(pc uint64, op OpCode, gas, cost *big.Int, memory *Memory, stack *stack, context *Context, err error) {
	if Debug {
		...
		object := context.self.(*state2.StateObject)
