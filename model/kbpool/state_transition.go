...
func ApplyMessage(env vm2.Environment, msg Message, coinbase *state2.StateObject) ([]byte, *big.Int, error) {
	return NewStateTransition(env, msg, coinbase).transitionState()
}

func NewStateTransition(env vm2.Environment, msg Message, coinbase *state2.StateObject) *StateTransition {
...
func (self *StateTransition) Coinbase() *state2.StateObject {
	return self.state.GetOrNewStateObject(self.coinbase)
}
func (self *StateTransition) From() (*state2.StateObject, error) {
	f, err := self.msg.From()
	if err != nil {
		return nil, err
	}
...
func (self *StateTransition) To() *state2.StateObject {
	if self.msg == nil {
		return nil
	}
	to := self.msg.To()
...
func (self *StateTransition) preCheck() (err error) {
	...
	if sender.Nonce() != msg.Nonce() {
		return block_error.NonceError(msg.Nonce(), sender.Nonce())
	}

	// Pre-pay gas / Buy gas of the coinbase account
	if err = self.BuyGas(); err != nil {
		if state2.IsGasLimitErr(err) {
			return err
		}
		return block_error.InvalidTxError(err)
	}
...
func (self *StateTransition) transitionState() (ret []byte, usedGas *big.Int, err error) {
	...
	if err = self.UseGas(IntrinsicGas(self.data)); err != nil {
		return nil, nil, block_error.InvalidTxError(err)
	}

	vmenv := self.env
	var ref vm2.ContextRef
	if MessageCreatesContract(msg) {
		...
		if err == nil {
			...
			if err := self.UseGas(dataGas); err == nil {
				...
				klog.Infof("Insufficient gas for creating code. Require", dataGas, "and have", self.gas)
			}
		}
	} else {
	...
	if err != nil && block_error.IsValueTransferErr(err) {
		return nil, nil, block_error.InvalidTxError(err)
	}

	if vm2.Debug {
		vm2.StdErrFormat(vmenv.StructLogs())
	}
