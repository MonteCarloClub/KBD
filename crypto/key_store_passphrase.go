...
func DecryptKeyFromFile(ks keyStorePassphrase, keyAddr common.Address, auth string) (keyBytes []byte, keyId [16]byte, err error) {
...
func decryptKeyV3(keyProtected *encryptedKeyJSONV3, auth string) (keyBytes []byte, keyId [16]byte, err error) {
	...
	keyId, _ = uuid.Parse(keyProtected.Id)
	...
	if !bytes.Equal(calculatedMAC, mac) {
		return nil, keyId, errors.New("Decryption failed: MAC mismatch")
	}
...
func decryptKeyV1(keyProtected *encryptedKeyJSONV1, auth string) (keyBytes []byte, keyId [16]byte, err error) {
	keyId, _ = uuid.Parse(keyProtected.Id)
	mac, err := hex.DecodeString(keyProtected.Crypto.MAC)
	if err != nil {
		return nil, keyId, err
