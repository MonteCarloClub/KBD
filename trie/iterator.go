...
func getType(node *common.Value) NodeType {
	if node.Len() == 0 {
		return EmptyNode
	}
...
func (self *Iterator) key(node *common.Value, path [][]byte) []byte {
	switch getType(node) {
	case LeafNode:
...
func (self *Iterator) next(node *common.Value, key []byte, path [][]byte) []byte {
