...
func ParanoiaCheck(t1 *Trie) (bool, *Trie) {
	t2 := New(common.Config.Db, "")

	t1.NewIterator().Each(func(key string, v *common.Value) {
		t2.Update(key, v.Str())
	})
...
func NewNode(key []byte, val *common.Value, dirty bool) *Node {
...
func (cache *Cache) Get(key []byte) *common.Value {
	...
	value := common.NewValueFromBytes(data)
...
func copyRoot(root interface{}) interface{} {
	...
	if b, ok := root.([]byte); ok {
		prevRootCopy = common.CopyBytes(b)
	} else {
		prevRootCopy = root
	}
...
func New(db common.Database, Root interface{}) *Trie {
...
func (self *Trie) setRoot(root interface{}) {
	...
	case string:
		/*
			if t == "" {
				root = crypto.Sha3(common.Encode(""))
			}
		*/
		self.Root = []byte(t)
	case []byte:
...
func (t *Trie) Get(key string) string {
	...
	k := CompactHexDecode(key)
	c := common.NewValue(t.getState(t.Root, k))
...
func (self *Trie) GetRoot() []byte {
	...
	case string:
		if t == "" {
			return sha3.Sha3(common.Encode(""))
		}
		return []byte(t)
	case []byte:
		if len(t) == 0 {
...
func (t *Trie) Cmp(trie *Trie) bool {
	return common.NewValue(t.Root).Cmp(common.NewValue(trie.Root))
}
...
func (t *Trie) getNode(node interface{}) *common.Value {
	n := common.NewValue(node)
	...
	if len(str) == 0 {
		...
		return common.NewValueFromBytes([]byte(str))
	}
...
func (it *TrieIterator) workNode(currentNode *common.Value) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
...
func (it *TrieIterator) Collect() [][]byte {
	...
	it.getNode(common.NewValue(it.trie.Root).Bytes())
...
type EachCallback func(key string, node *common.Value)

func (it *TrieIterator) Each(cb EachCallback) {
	it.fetchNode(nil, common.NewValue(it.trie.Root).Bytes(), cb)
}
...
func (it *TrieIterator) iterateNode(key []byte, currentNode *common.Value, cb EachCallback) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
